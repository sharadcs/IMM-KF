function output = viterbi_kalman2(x_0,P_0,N,meas,W,system,n,use_constraints)
...
ndivs = 5;  % # of divisions of sample to check paths
          ...
          if M_tr(i,j)
              ...
              if i~=j
                  ...
                  for t_bar = (ts/ndivs)/2:ts/ndivs:ts-(ts/ndivs)/2
                      F_hat = c2d_switch(t_bar,ts,A{mode1},A{mode2});
                      G_hat = input_switch(t_bar,ts,A{mode1},A{mode2},B{mode1},B{mode2});
