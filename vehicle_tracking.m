...
for k=1:K
    ...
    if ~isempty(curr_switches);
        ...
        G(:,:,k) = input_switch(curr_switches-(tk-ts),A{sigma(i)},A{sigma(i+1)},B{sigma(i)},B{sigma(i+1)});
        i=i+1;
    else
        F(:,:,k) = expm(A{sigma(i)}*ts);
        G(:,:,k) = inv(A{sigma(i)})*(F(:,:,k)-eye(2))*B;
...
for k=1:size(y,2)
    ...
    for mode2 = 1:n_states
        if mode1 ~= mode2
            for t_bar = (ts/ndivs)/2:ts/ndivs:ts-(ts/ndivs)/2
                ...
                G_hat = input_switch(t_bar,A{mode1},A{mode2},B{mode1},B{mode2});
    ...
    if k<=W
        ...
        if ~viterbi_use_constraints
            for i=1:n_states
                viterbi_init.x{i} =Truth_KF.x(:,k-W);%-W+1);%viterbi_KF.x(:,k-W);% %viterbi_bank.x{i};%
                viterbi_init.P{i} =Truth_KF.P(:,:,k-W);%-W+1);%100*viterbi_KF.P(:,:,k-W);% %5*viterbi_bank.P{i};%
            end
        else
            for i=1:n_states*n_states
